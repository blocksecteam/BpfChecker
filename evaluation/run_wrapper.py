...
def run_fuzzing(genreator_path, rbpf_runner_path, output_base_path, loop_index) -> bool:
    ...
    output_dir =  os.path.abspath(os.path.join(output_base_path, f"output_{datetime.now().strftime('%Y%m%d%H%M%S')}_{loop_index}"))
    generator_command = [genreator_path, "-o", output_dir, "-n", "1000"]
    runner_command = [rbpf_runner_path, "--mode", "batch", os.path.join(output_dir, "program_set"), "--only_vm", "-t"]

    # Generate corpus
    subprocess.run(generator_command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
